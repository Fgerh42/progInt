...
export default {
  ...
  async list(dados) {
    ...
    await api.post(`financeiro_get`,{access_token:data.data.token})
  ...
  async save(dados) {
    ...
    await api.post(`financeiro_add`,dados)
      .then(function(response) {
        if (response.status === 200) {
          retorno.ok = 1 
          retorno.dados = response.data.data
  ...
  async delete(dados) {
    ...
    await api.post(`financeiro_delete`,{access_token:data.data.token,id:dados})
    .then(function(response) {
      if (response.status === 200) {
        retorno.ok = 1 
        retorno.dados = response.data.data
  ...
  async edit(dados) {
    ...
    dados.access_token = data.data.token
    await api.post(`financeiro_update`,dados)
      .then(function(response) {
        if (response.status === 200) {
          retorno.ok = 1 
          retorno.dados = response.data.data
